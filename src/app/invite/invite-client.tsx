    ...
    <div className="min-h-screen bg-[#0F1626] flex items-center justify-center p-4">
      <div className="bg-white/5 border border-white/10 rounded-2xl p-8 max-w-md w-full">
        ...
        <form onSubmit={handleAccept} className="space-y-4">
          ...
          <div>
              <label className="block text-sm font-medium text-slate-300 mb-1">{userExists ? 'Mot de passe de votre compte' : 'Créez votre mot de passe'}</label>
              <input 
                type="password" 
                value={password}
              ...
              />
              <p className="text-xs text-slate-500 mt-2">{userExists ? 'Il confirme que cette invitation vous appartient.' : 'Ce mot de passe vous permettra de vous connecter à Kobara.'}</p>
          </div>

          {userExists && (
            <p className="text-sm text-emerald-400 bg-emerald-400/10 p-3 rounded-xl text-center">
              Un compte existe déjà avec cette adresse. Saisissez son mot de passe pour accepter l&apos;invitation.
            </p>
          )}
