...
function PaymentProcessingContent() {
  ...
  const [message, setMessage] = useState('Nous attendons la confirmation sécurisée de votre paiement.');
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        ...
        if (result.status === 'succeeded' && result.redirectUrl) {
          ...
          finish(result.redirectUrl);
          return;
        }
        if (['failed', 'expired', 'canceled'].includes(result.status)) {
          finish(result.redirectUrl || `/pay/error?reason=${result.status}`);
          return;
        }
      ...
      if (Date.now() >= deadline) {
        finish('/pay/error?reason=expired');
        return;
      }
    ...
    const countdownTimer = setInterval(() => {
      ...
      if (remaining === 0 && !stopped) finish('/pay/error?reason=expired');
    }, 1000);
    poll();
  ...
  return (
    <main className="flex min-h-[100dvh] items-center justify-center bg-[#070b12] p-6 text-white">
      <section className="w-full max-w-md text-center">
        <div className="mx-auto flex h-16 w-16 items-center justify-center rounded-lg border border-orange-500/30 bg-orange-500/10">
